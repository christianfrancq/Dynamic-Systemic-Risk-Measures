...
estimGARCHbiv<- function(P, omega.init,mat.alpha.init,beta.init,eps, niv.alpha, niv.alpha.prim, 
             petit=sqrt(.Machine$double.eps), r0=10, alpha0=0.10, B=100)
     {
n <- length(eps[,1])
omega<-c(0,0)
beta<-omega
...
omeg<-theta1.star[1]; alph1<-theta1.star[2]; alph2<-theta1.star[3]; bet<-theta1.star[4]
for(t in 2:n){
...
list(lower.CoVaR=lower.CoVaR,CoVaR=CoVaR,upper.CoVaR=upper.CoVaR,lower.CoVaR.norm=lower.CoVaR.norm,CoVaR.norm=CoVaR.norm,
upper.CoVaR.norm=upper.CoVaR.norm,lower.boot=lower.boot,upper.boot=upper.boot, VaR=VaR)
}
...
log_returns <- rend[(first-1):(last-1),1]  # actual log-returns
estimated_covar <- est$CoVaR[(first-1):(last-1)] # actual CoVaR values
SP_returns <- rend[(first-1):(last-1),2]  # 
VaR <- est$VaR[(first-1):(last-1)]  # 


# Identify intersection points (only where the log-return goes below -Estimated CoVaR)
intersection_points_rouge <- which((log_returns < -estimated_covar)&(SP_returns < -VaR))
intersection_points_vert <- which((log_returns >= -estimated_covar)&(SP_returns < -VaR))
intersection_points_orange <-  which((log_returns < -estimated_covar)&(SP_returns >= -VaR))
...
# Data frame for intersections (only plot points where returns go below CoVaR)
intersections_rouge <- data[intersection_points_rouge, ]
intersections_vert <- data[intersection_points_vert, ]
intersections_orange <- data[intersection_points_orange, ]
...
p1<-ggplot(data, aes(x = Date)) +
 ...
 geom_point(data = intersections_rouge, aes(x = Date, y = Estimated_CoVaR), color = "red", size = 0.5, shape = 21, fill = "red", stroke = 1.5) +  
 geom_point(data = intersections_vert, aes(x = Date, y = Estimated_CoVaR), color = "green", size = 0.5, shape = 21, fill = "green", stroke = 1.5) +  
 geom_point(data = intersections_orange, aes(x = Date, y = Estimated_CoVaR), color = "orange", size = 0.5, shape = 21, fill = "orange", stroke = 1.5) +  
labs(title = "",
...
ggsave("figure2_1.pdf", plot = p1, width = 8, height = 4)  


# Plot
p2<-ggplot(data, aes(x = Date)) +
  geom_line(aes(y = SP_Returns, color = "SP500 Log-Returns"), size = 0.5) + 
  geom_line(aes(y = Estimated_VaR, color = "-Estimated VaR"), size = 0.5) +
 geom_point(data = intersections_rouge, aes(x = Date, y = Estimated_VaR), color = "red", size = 0.5, shape = 21, fill = "red", stroke = 1.5) +  
 geom_point(data = intersections_vert, aes(x = Date, y = Estimated_VaR), color = "green", size = 0.5, shape = 21, fill = "green", stroke = 1.5) +  
 geom_point(data = intersections_orange, aes(x = Date, y = Estimated_VaR), color = "orange", size = 0.5, shape = 21, fill = "orange", stroke = 1.5) +  
labs(title = "",
       x = "Time (Backtest Period)",
       y = "Value",
       color = "",
       fill = "") +
  scale_color_manual(values = c("SP500 Log-Returns" = "black",  "-Estimated VaR" = "blue")) +
  theme_bw() +
  theme(legend.text = element_text(size = 6))

ggsave("figure2_2.pdf", plot = p2, width = 8, height = 4)  
