...
#nn<-10000000
#norm<-matrix(rnorm(2*nn),nrow=nn,ncol=2)
#norm[,2]<-rho*norm[,1]+sqrt(1-rho^2)*norm[,2]
#var(norm) 
#quantile(norm[,2],niv.alpha.prim)
#xi<-quantile(norm[,2],niv.alpha.prim)
#indic<-which(norm[,2]<=xi)
#quantile(norm[indic,1],niv.alpha)
#{if(nu==Inf|nu<2){
#ct<-1}
#else
#{ct<-sqrt((nu-2)/nu)
...
#var(eta) 
#quantile(eta[,2],niv.alpha.prim)
#xi<-quantile(eta[,2],niv.alpha.prim)
#indic<-which(eta[,2]<=xi)
#quantile(eta[indic,1],niv.alpha)
#####################################################
########### to compute the critical value and probability of rejection of the randomized test when
############  the discrete test statistic is equal to the critical value 
####################################################################
Nrep<-20000
p<-niv.alpha*niv.alpha.prim
# the randomized UC test of exact level alpha0=0.05 can be obtained by exact calculation
...
alpha0<-0.05
#qchisq(0.95, df=1)
# the randomized Independence test is approximated by simulations
set.seed(13)
...
# check that the randomized tests have good levels
set.seed(13)

res<-replicate(Nrep,simula(n, P, niv.alpha, niv.alpha.prim ,alpha0, crit.UC, eps.plus, eps.moins, prob.crit, crit.Ind, eps.plus.ind, eps.moins.ind, prob.crit.ind))
...
length(which(res[1,]==1))*100/Nrep
length(which(res[2,]==1))*100/Nrep
length(which(res[3,]==1))*100/Nrep # bon niveau UC
length(which(res[4,]==1))*100/Nrep # bon niveau Ind
...
res<-replicate(Nrep,simulation.mod(n, P, omega,  mat.alpha, beta, rho, u.eta, crit.UC, eps.plus, eps.moins, prob.crit, crit.Ind, eps.plus.ind, eps.moins.ind, prob.crit.ind, nu))


length(which(res[1,]==1))*100/Nrep
length(which(res[2,]==1))*100/Nrep
length(which(res[3,]==1))*100/Nrep 
length(which(res[4,]==1))*100/Nrep
...
#################################### the same with Gaussian innovations 
nu<-Inf
n<-1000
set.seed(13)
