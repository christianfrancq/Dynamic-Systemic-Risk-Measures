...
years <- time_series[first:last]
length(years)
values<-numeric()
length(values)<-4*length(years)
str(values)
...
Size[i]<-sum(caps[,i])
if(i==1)values[22:n]<-est$ratio[21:(n-1)]
if(i==2)values[(n+22):(2*n)]<-est$ratio[21:(n-1)]
if(i==3)values[(2*n+22):(3*n)]<-est$ratio[21:(n-1)]
if(i==10)values[(3*n+22):(4*n)]<-est$ratio[21:(n-1)]
}


# 
data <- data.frame(
  Time = rep(years, times = 4),
  Value = values,
  Institution = rep(c("J.P. Morgan", "Unicredit", "Goldman Sachs", "Credit Agricole"), each = n)
)
str(data)

# Create the plot
p<-ggplot(data, aes(x = Time, y = Value, color = Institution)) +
  geom_line(size = 0.5) +  # Line plot for each institution
  labs(title = "",
       x = "Time",
       y = "Value",
       color = "Institution") +  # Axis labels and legend title
  scale_y_continuous(limits = c(0.45, 2.0)) +  # Set y-axis limits
  theme_bw()  +  # Clean, minimal theme
  theme(
    plot.title = element_text(hjust = 0.5),  # Center the plot title
legend.text = element_text(size = 6)   
)

ggsave("figure4.pdf", plot = p, width = 8, height = 4)  
